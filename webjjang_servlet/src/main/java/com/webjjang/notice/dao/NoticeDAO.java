...
public class NoticeDAO extends DAO {
	...
	public List<NoticeVO> list(PageObject pageObject) throws Exception {
		...
		try {
			...
			System.out.println("NoticeDAO.list() = " + getListSQL(pageObject) );
	...
	public int update(NoticeVO vo) throws Exception {
		...
		try {
			...
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
				...
				throw new Exception("예외 발생 : 공지 수정 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public int delete(Long no) throws Exception {
		...
		try {
			...
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
				...
				throw new Exception("예외 발생 : 공지 삭제 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	private String getPeriod(PageObject pageObject) {
		...
		sql += " and ( 1=1 ";
			...
			else if (period.equals("res")) // 예약공지
	...
	final String VIEW = "select no, title, content, " + " to_char(startDate, 'yyyy-mm-dd') startDate, "
			+ " to_char(endDate, 'yyyy-mm-dd') endDate, " + " to_char(updateDate, 'yyyy-mm-dd') writeDate, "
	...
	final String UPDATE = "update notice " + " set title = ?, content = ?, startDate = ?, endDate = ? "
			+ " where no = ?";
