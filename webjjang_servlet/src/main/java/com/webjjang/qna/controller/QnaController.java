	...
	public String execute(HttpServletRequest request) {
		...
		try { // 정상 처리
			...
			case "/qna/questionForm.do":
				System.out.println("3-1. 질문 글등록 폼");
			...
			case "/qna/answerForm.do":
				System.out.println("3-2. 답변 글등록 폼");
				// 넘어온 글 번호에 따른 데이터를 가져와서 request에 저장한다
			...
			case "/qna/write.do":
				...
				if(strParentNo != null && !strParentNo.equals(""))
					vo.setRefNo(Long.parseLong(strParentNo));
				...
				Execute.execute(Init.get(uri), vo);
				 ...
				 // 메시지 설정
	             if (vo.isQuestion()) {
	                 session.setAttribute("msg", "질문이 등록 되었습니다.");
	             } else {
	                 session.setAttribute("msg", "답변이 등록 되었습니다.");
	             }
